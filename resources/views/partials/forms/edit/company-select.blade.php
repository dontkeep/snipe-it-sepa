...
@if (($snipeSettings->full_multiple_companies_support=='1') && (!Auth::user()->isSuperUser()))
    ...
    <div class="form-group">
        <label for="{{ $fieldname }}" class="col-md-3 control-label {{ (isset($required) && $required) ? 'required' : '' }}">
            {{ $translated_name }}
...
@else
    ...
    <div id="{{ $fieldname }}" class="form-group{{ $errors->has($fieldname) ? ' has-error' : '' }}">
        ...
        <div class="col-md-8">
            <select class="js-data-ajax" data-endpoint="companies" data-placeholder="{{ trans('general.select_company') }}" name="{{ $fieldname }}" style="width: 100%" id="company_select"{{ (isset($multiple) && ($multiple=='true')) ? " multiple='multiple'" : '' }}{{ (isset($required) && $required) ? ' required' : '' }}>
                @isset ($selected)
